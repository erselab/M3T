...
if(Municipal_file == "CWNS"){
  ...
  coordinates(Municipal_flow) <- ~Facility_Longitude + Facility_Latitude
  proj4string(Municipal_flow) <- CRS(SRS_string="EPSG:4326")
...
}else if(Municipal_method=="Moore_EF"){
  if(Municipal_file=="CWNS"){
...
}else if(Municipal_method=="Moore_linear"){
...
# Rasterise
Municipal_flow_crop <- crop(Municipal_flow,d03_rast)
Municipal_flow_crop_filt <- subset(Municipal_flow_crop,!is.na(emiss))
central_rast <- rasterize(Municipal_flow_crop_filt, d03_rast, "emiss", fun=sum)
...
write.csv(Municipal_flow_crop_filt, file.path(Output_directory,"WWTP_municipal_all.csv"))

# Now just the names, coordinates and emissions
if(Municipal_file=="CWNS"){
  Municipal_flow_crop_output <- data.frame(Municipal_flow_crop_filt$FACILITY_NAME,coordinates(Municipal_flow_crop_filt),Municipal_flow_crop_filt$emiss)
}else if(Municipal_file=="DMR"){
  Municipal_flow_crop_output <- data.frame(Municipal_flow_crop_filt$Facility_Name,coordinates(Municipal_flow_crop_filt),Municipal_flow_crop_filt$emiss)
}
names(Municipal_flow_crop_output) <- c('Site_Name','Longitude','Latitude','Emission_mol_per_s')
