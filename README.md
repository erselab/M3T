...
1.  Process_X
    - Each sector has a TRUE/FALSE option for running the sector, e.g.,
      Process_wetlands_and_inland_waters.
2.  Source_X
    - Input data can generally be set to M3T (built in data, generally
      the default), download to download the data from the source, or a
      filepath to use a local copy of the needed dataset. It is assumed
      the formatting is consistent with the downloaded data. E.g.,
      Source_GHGRP_combustion.
3.  Use_ACES and Use_Vulcan
    ...
    - See publications for [ACES](https://doi.org/10.1002/2017JD027359)
      and [Vulcan](https://doi.org/10.1038/s41597-025-06391-w). Datasets
...
1.  Landfills
      ...
      - **method**
        ...
        - **Collection_first**
          - Uses GHGRP method HH-8 emissions which are based on an
            assumed landfill gas capture efficiency and the measured
    ...
    - GHGRP_MSW_Landfills.csv
      - GHGRP facility data used for the run (i.e., after filtering to
        the domain and year) including the calculated emissions
    - LMOP_MSW_Landfills.csv
      - LMOP facility data used for the run (i.e., after filtering to
        the domain and year) including the calculated emissions
2.  Natural Gas Distribution
      ...
      - **type**
        ...
        - **post_meter**
          - all emissions after the gas has passed the gas meter
            including leaks from furnaces, stoves, water heaters,
            pipelines, etc.
        - **MnR**
      ...
      - **scale**
        ...
        - **national**
          - calculated using the fraction of open / low density urban
            land cover relative to the national total in combination
            with
    - GHGRP_industrial_watewater_treatment.csv
      - GHGRP facility data used for the run (i.e., after filtering to
        the domain and year) including the emissions
    - Municipal_watewater_treatment.csv
      - Facility data including flow used for the run (i.e., CWNS and/or
        DMR data after filtering to the domain and year) including the
        calculated emissions
6.  Wetlands and Inland Waters
    - Wetcharts_NLCD_downscaled_subset\_**N**.nc
      - Wetland emissions from WetCHARTs downscaled from 0.5 degrees to
...
8.  Combining sectors
    - a separate folder will be created with each unique combination of
      sectors saved numerically if that is set in the config. A csv will
      also be saved which provides a clear legend detailing which
      variations are in each inventory file. Summary combinations will
      be saved in a separate folder, as will thermogenic/non-thermogenic
      combinations if set in the config.
